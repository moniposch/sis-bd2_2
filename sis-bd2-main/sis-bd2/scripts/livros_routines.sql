...
/*!50001 VIEW `vw_livro_autor` AS select `li`.`titulo` AS `titulo`,`li`.`autor` AS `autor` from `livros` `li` where (`li`.`autor` is not null) group by `li`.`autor` order by `li`.`autor` */;
